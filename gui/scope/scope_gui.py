...
class ScopeMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self, **kwargs):
        ...
        ui_file = os.path.join(this_dir, 'ui_scope_gui.ui')
...
class ScopeSelectionDialog(QtWidgets.QDialog):
    ...
    def __init__(self, **kwargs):
        # Get the path to the *.ui file
        this_dir = os.path.dirname(__file__)
        ui_file = os.path.join(this_dir, 'scope_selector_dialog.ui')
...
class ScopeGui(GUIBase):
    ...
    def on_activate(self):
        ...
        self._scope_logic = self._scope_logic_con()
        ...
        hw_constr = self._scope_logic.streamer_constraints
        ...
        self._mw.trace_snapshot_Action.triggered.connect(
            self._scope_logic.save_trace_snapshot, QtCore.Qt.QueuedConnection)
        ...
        self.sigStartCounter.connect(
            self._scope_logic.start_reading, QtCore.Qt.QueuedConnection)
        self.sigStopCounter.connect(
            self._scope_logic.stop_reading, QtCore.Qt.QueuedConnection)
        self.sigStartRecording.connect(
            self._scope_logic.start_recording, QtCore.Qt.QueuedConnection)
        self.sigStopRecording.connect(
            self._scope_logic.stop_recording, QtCore.Qt.QueuedConnection)
        self.sigSettingsChanged.connect(
            self._scope_logic.configure_settings, QtCore.Qt.QueuedConnection)

        ##################
        # Handling signals from the logic
        self._scope_logic.sigDataChanged.connect(
            self.update_data, QtCore.Qt.QueuedConnection)
        self._scope_logic.sigSettingsChanged.connect(
            self.update_settings, QtCore.Qt.QueuedConnection)
        self._scope_logic.sigStatusChanged.connect(
    ...
    def on_deactivate(self):
        ...
        self._scope_logic.sigDataChanged.disconnect()
        self._scope_logic.sigSettingsChanged.disconnect()
        self._scope_logic.sigStatusChanged.disconnect()
    ...
    def apply_channel_settings(self, update_logic=True):
        ...
        ch_list = self._scope_logic.active_channels
    ...
    def keep_former_channel_settings(self):
        """
        """
        curr_channels = self._scope_logic.active_channel_names
        curr_av_channels = self._scope_logic.averaged_channel_names
        for chnl, widgets in self._csd_widgets.items():
    ...
    def update_data(self, data_time=None, data=None, smooth_time=None, smooth_data=None):
        ...
        if data_time is None and data is None and smooth_data is None and smooth_time is None:
            data_time, data = self._scope_logic.trace_data
            smooth_time, smooth_data = self._scope_logic.averaged_trace_data
        ...
        if curr_value_channel != 'None':
            if curr_value_channel.startswith('average '):
                ...
                val = data[chnl][-1]
            ch_type = self._scope_logic.active_channel_types[chnl]
            ch_unit = self._scope_logic.active_channel_units[chnl]
    ...
    def update_status(self, running=None, recording=None):
        ...
        """
        if running is None:
            running = self._scope_logic.module_state() == 'locked'
        if recording is None:
            recording = self._scope_logic.data_recording_active
    ...
    def update_settings(self, settings_dict=None):
        if settings_dict is None:
            settings_dict = self._scope_logic.all_settings
    ...
    def _toggle_channel_data_plot(self, channel, show_data, show_average):
        ...
        if show_average and average_enabled and self._scope_logic.moving_average_width > 1:
            if channel in self._channels_per_axis[0]:
